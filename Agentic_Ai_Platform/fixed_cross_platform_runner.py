...
def extract_tool_calls(response_content: str) -> List[Dict[str, Any]]:
    """Extract tool calls from response - FIXED VERSION"""
    tool_calls = []
    
    # Pattern 1: Tool calls with parameters (handles empty params)
    ...
    for match in matches:
        ...
        try:
            ...
            # If JSON parsing fails, still try to extract with empty params
            tool_calls.append({
                "tool_name": tool_name,
                "params": {},
                "full_text": match.group(0)
    ...
    # Pattern 2: Tool calls without explicit parameters block
    if not tool_calls:
        simple_pattern = r"I need to use the tool: ([a-zA-Z0-9_:]+)(?:\s*\n|$)"
        matches = re.finditer(simple_pattern, response_content, re.MULTILINE)
        for match in matches:
            tool_name = match.group(1).strip()
            tool_calls.append({
                "tool_name": tool_name,
...
class AsyncToolIntegratedExecutor:
    ...
    async def _execute_agent_with_tools(self, task: AgentTask) -> Dict[str, Any]:
        """Execute agent with tool support - FIXED VERSION"""
        try:
