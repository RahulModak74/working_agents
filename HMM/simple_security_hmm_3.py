    ...
    def visualize_results(self, save_results=True):
        ...
        try:
            ...
            if save_results:
                ...
                # Save summary statistics with proper serialization
                summary_stats = {
                    'total_sessions': int(len(results_df)),
                    'high_risk_sessions': int((results_df['attack_probability'] > 0.7).sum()),
                    'medium_risk_sessions': int(((results_df['attack_probability'] > 0.3) & 
                                            (results_df['attack_probability'] <= 0.7)).sum()),
                    'low_risk_sessions': int((results_df['attack_probability'] <= 0.3).sum()),
                    'mean_attack_probability': float(results_df['attack_probability'].mean()),
...
def main():
    ...
    try:
        ...
        if results_df is not None:
            ...
            print("   ⚠️ Validation skipped due to visualization errors")
